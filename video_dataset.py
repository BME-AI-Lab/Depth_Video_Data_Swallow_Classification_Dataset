...
#Source code edited specifically for depth video dataset
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def _load_image(self, directory: str, idx: int) -> np.array:
        # return Image.open(os.path.join(directory, self.imagefile_template.format(idx))).convert('RGB')
        return np.load(os.path.join(directory, self.imagefile_template.format(idx)))
...
class NplistToTensor(torch.nn.Module):
    """
    Converts a list of PIL images in the range [0,255] to a torch.FloatTensor
    of shape (NUM_IMAGES x CHANNELS x HEIGHT x WIDTH) in the range [0,1].
    Can be used as first transform for ``VideoFrameDataset``.
    ...
    def forward(
        ...
        result = torch.stack(img_list, dim=1)
        for i in img_list:
            del i
        return result  # .cuda()
